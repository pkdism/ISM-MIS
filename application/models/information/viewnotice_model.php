...
class Viewnotice_model extends CI_Model
{
	...
	function get_notices()
	{
		...
		$where = "notice_cat = 'all' OR notice_cat = '".$notice_cat."'";
		$this->db->where($where);
		$query = $this->db->select("info_notice_archieve_details.*, user_details.*, auth_types.type as auth_name, departments.name as department, designations.name as designation")
						  ->from($this->table)
						  ...
						  ->order_by("info_notice_archieve_details.posted_on", "desc")
						  ->get();
