...
window.onload = function(){
		...
		/* Function called on clicking on a date: */
...
/*
	...
	useMode (Integer) â€" Possible values are 1 and 2 as follows:
		1 â€" The calendar's HTML will be directly appended to the field supplied by target
		2 â€" The calendar will appear as a popup when the field with the id supplied in target is clicked.
	
	target (String) â€" The id of the field to attach the calendar to , usually a text input field when using useMode 2.
	
	isStripped (Boolean) â€" When set to true the calendar appears without the visual design - usually used with useMode 1
	
	selectedDate (Object) â€" When supplied , this object tells the calendar to open up with this date selected already.
	
	yearsRange (Array) â€" When supplied , this array sets the limits for the years enabled in the calendar.
	
	limitToToday (Boolean) â€" Enables you to limit the possible picking days to today's date.
	
	cellColorScheme (String) â€" Enables you to swap the colors of the date's cells from a wide range of colors.
