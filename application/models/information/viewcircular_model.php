...
class Viewcircular_model extends CI_Model
{
	...
	function get_circulars()
	{
		...
		$where = "circular_cat = 'all' OR circular_cat = '".$circular_cat."'";
		$this->db->where($where);
		$query = $this->db->select("info_circular_archieve_details.*, user_details.*, auth_types.type as auth_name, departments.name as department, designations.name as designation")
						  ->from($this->table)
						  ...
						  ->order_by("info_circular_archieve_details.posted_on", "desc")
						  ->get();
